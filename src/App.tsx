...
export default function App() {
  ...
  const exportJson = () => {
    ...
    a.href = url;
    a.download = `savings_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  const expenseCats = useMemo(
    () => [...DEFAULT_EXPENSE_CATS, ...(d.customExpense || [])],
    [d.customExpense]
  );
  ...
  useEffect(() => {
    if (d.accounts.length >= 2) {
      setFFrom(d.accounts[0].id);
      setFTo(d.accounts[1].id);
    }
  }, [d.accounts]);
  ...
  const accountBalances = useMemo(() => {
    ...
    d.accounts.forEach((a) => {
      map[a.id] = a.initial || 0;
    });
    (d.txList || []).forEach((t) => {
      if (t.type === 'income') {
        map[t.acct] = (map[t.acct] || 0) + t.amount;
      }
      if (t.type === 'expense') {
        map[t.acct] = (map[t.acct] || 0) - t.amount;
      }
  ...
  const totalBalance = useMemo(
    () => d.accounts.reduce((s, a) => s + (accountBalances[a.id] || 0), 0),
    [d.accounts, accountBalances]
  );
  ...
  const monthlyData = useMemo(() => {
    ...
    (d.txList || [])
      .filter(
        (t: any) =>
          t.type !== 'transfer' &&
          (chartAcct === 'all' || String(t.acct) === String(chartAcct))
      )
      .forEach((t: any) => {
    ...
    return Object.values(map)
      ...
      .map((m: any) => ({ ...m, label: m.month.replace('-', '/') }));
  }, [d.txList, chartAcct]);
  ...
  const balanceHistory = useMemo(() => {
    const accts =
      chartAcct === 'all'
        ? d.accounts
        : d.accounts.filter((a) => String(a.id) === String(chartAcct));
    const initMap = {};
    accts.forEach((a) => {
      initMap[a.id] = a.initial || 0;
    });
    const sorted = [...(d.txList || [])].sort((a, b) =>
      a.date.localeCompare(b.date)
    );
    const points = [
      {
        label: '開始',
        ...Object.fromEntries(accts.map((a) => [a.name, initMap[a.id]])),
      },
    ];
    const cur = { ...initMap };
    sorted.forEach((t) => {
      ...
      if (t.type === 'transfer') {
        cur[t.from] = (cur[t.from] || 0) - t.amount;
        cur[t.to] = (cur[t.to] || 0) + t.amount;
      }
      const relevant = accts.some(
        (a) => t.acct === a.id || t.from === a.id || t.to === a.id
      );
      if (relevant)
        points.push({
          label: t.date.slice(5),
          ...Object.fromEntries(accts.map((a) => [a.name, cur[a.id] || 0])),
        });
    });
    return points;
  }, [d, chartAcct]);
  ...
  const addTx = () => {
    ...
    if (fType === 'transfer') {
      tx.from = Number(fFrom);
      tx.to = Number(fTo);
    } else {
      tx.acct = Number(fAcct);
      tx.cat = fCat;
    }
  ...
  const delTx = (id) =>
    setD((p) => ({
      ...p,
      txList: (p.txList || []).filter((t) => t.id !== id),
    }));
  ...
  const addAccount = () => {
    ...
    setD((p) => ({
      ...
      accounts: [
        ...p.accounts,
        { id, name, initial: parseFloat(newAcctInit) || 0 },
      ],
    }));
    setNewAcctName('');
    setNewAcctInit('0');
  ...
  const delAccount = (id) => {
    setD((p) => ({
      ...
      txList: (p.txList || []).filter(
        (t) => t.acct !== id && t.from !== id && t.to !== id
      ),
    }));
  };
  ...
  const updateInitial = (id, val) => {
    setD((p) => ({
      ...
      accounts: p.accounts.map((a) =>
        a.id === id ? { ...a, initial: parseFloat(val) || 0 } : a
      ),
    }));
  };
  ...
  const acctName = (id) =>
    (d.accounts.find((a) => a.id === id) || {}).name || '不明';
  ...
  const mIncome = (d.txList || [])
    .filter((t) => t.date.startsWith(thisM) && t.type === 'income')
    .reduce((s, t) => s + t.amount, 0);
  const mExpense = (d.txList || [])
    .filter((t) => t.date.startsWith(thisM) && t.type === 'expense')
    .reduce((s, t) => s + t.amount, 0);
      ...
      <div style={{ padding: 14, maxWidth: 700, margin: '0 auto' }}>
        ...
        {tab === 'dashboard' && (
          <>
                ...
                >
                  ...
                  <div
                    style={{
                      fontSize: 18,
                      fontWeight: 700,
                      color: accountBalances[a.id] >= 0 ? '#1e293b' : '#ef4444',
                    }}
                  >
                    {disp(accountBalances[a.id] || 0)}
                  </div>
                </div>
              ))}
            </div>
            ...
            >
              {[
                { label: '今月の収入', value: mIncome, color: '#10b981' },
                { label: '今月の支出', value: mExpense, color: '#ef4444' },
              ].map((s) => (
            ...
            >
              {[{ id: 'all', name: 'すべて' }, ...d.accounts].map((a, i) => (
                <button
                  key={a.id}
                  onClick={() => setChartAcct(String(a.id))}
                  style={{
                    padding: '5px 14px',
                    borderRadius: 20,
                    border: 'none',
                    ...
                    fontWeight: 600,
                    background:
                      String(chartAcct) === String(a.id)
                        ? i === 0
                          ? '#6366f1'
                          : ACCOUNT_COLORS[(i - 1) % ACCOUNT_COLORS.length]
                        : '#e2e8f0',
                    color:
                      String(chartAcct) === String(a.id) ? '#fff' : '#64748b',
                  }}
                >
                  {a.name}
                </button>
            ...
            {/* 残高推移 */}
            <div
              style={{ ...card, ...(hideAmt ? { background: '#1e293b' } : {}) }}
            >
              <div
                style={{
                  fontWeight: 700,
                  marginBottom: 10,
                  color: hideAmt ? '#fff' : 'inherit',
                }}
              >
                残高推移
              </div>
                ...
                <ResponsiveContainer width="100%" height={190}>
                  <LineChart data={balanceHistory}>
                    ...
                    {(chartAcct === 'all'
                      ? d.accounts
                      : d.accounts.filter(
                          (a) => String(a.id) === String(chartAcct)
                        )
                    ).map((a, i) => (
                      <Line
                        key={a.id}
              ...
              <>
                ...
                <Field label="カテゴリ">
                  <select
                    ...
                    style={iStyle}
                  >
                    {(fType === 'income' ? incomeCats : expenseCats).map(
                      (c) => (
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                  <select
                    ...
                    style={{ ...iStyle, width: 70, flex: 'none' }}
                  >
        ...
        {tab === 'history' && (
          <>
            ...
            >
              ...
              <select
                value={fFType}
                onChange={(e) => setFFType(e.target.value)}
              ...
              <select
                value={fFAcct}
                onChange={(e) => setFFAcct(e.target.value)}
                style={{ ...iStyle, flex: 1, minWidth: 100 }}
              >
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>
                        {isTransfer
                          ? `${acctName(t.from)} → ${acctName(t.to)}`
                          : `${t.cat} ／ ${acctName(t.acct)}`}
                      </div>
          ...
          <div style={card}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <input
                ...
                placeholder="口座名（例：現金）"
                style={{ ...iStyle, flex: 2 }}
              />
              <input
                type="number"
                value={newAcctInit}
                onChange={(e) => setNewAcctInit(e.target.value)}
                placeholder="初期残高"
                style={{ ...iStyle, flex: 1 }}
              />
              <button
            ...
            >
              ...
              <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 6 }}>
                全取引・口座・カテゴリデータをJSONファイルとして保存します
              </div>
            </div>
          </div>
        )}
